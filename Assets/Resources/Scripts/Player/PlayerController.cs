...
namespace com.dankstudios
{
    public class PlayerController : Bolt.EntityEventListener<IPlayerState>
    {
        ...
        void Update()
        {
            ...
            // 404 - I have no idea what this does and its throwing errors because its not compiled in bolt prefabs so we'll leave it out for now.
        ...
        void PollKeys(bool mouse)
        {
            ...
            if (Input.GetKeyDown(KeyCode.B))
            {
                if (!menu)
                {
                    Debug.Log("Mouse Should Be Locked");
                    Cursor.visible = true;
                    menu = true;

                    // 404 - Throwing errors until I re-add my inventory system we ported from village project
                    //this.GetComponent<UIController>().showInventory();
                }
                else
                {
                    Debug.Log("Mouse Should NOT Be Locked");
                    Cursor.visible = true;
                    menu = false;

                    // 404 - Throwing errors until I re-add my inventory system we ported from village project
                    //this.GetComponent<UIController>().hideInventory();
                }
            }
        ...
        void WeaponChanged()
        {
            ...
            {
                // 404 - commented out due to error
